...
// Listener is implemented by layouted components that want to be
// informed that their layout has changed.
type Listener interface {

	// Layouted is called back after a layout change.  If its returned
	// value is true the component layout is recalculated.
	Layouted() bool
}
...
type Chainer interface {
	...
	// ForStacked provides the chained Dimer in the order they are
	// stacked.
...
var ErrNil = fmt.Errorf("%w"+"nil-Dimer: ", ErrLyt)
var ErrNotFound = fmt.Errorf("%w"+"Dimer not found: ", ErrLyt)
var StrErrNil = "given Dimer must not be nil"
var StrErrNotFound = "given Dimer cannot be found in layout"
...
// a margin.
type Manager struct{ Root Dimer }
...
func (m *Manager) Reflow(dirty func(Dimer)) (err error) {
	...
	forDimer(m.Root, func(d Dimer) (stop bool) {
	...
	// if there is no callback we make anyway sure that Dimer with
	// changed visible area are flagged as dirty.
	if dirty == nil {
		forDimer(m.Root, func(d Dimer) (stop bool) {
	...
	// flag the Dimer with changed visible are as dirty and provide them
	// to given callback.
	forDimer(m.Root, func(d Dimer) (stop bool) {
		d.Dim().finalizeLayout(ddBefore[d])
		if !d.Dim().IsDirty() {
...
func forDimer(d Dimer, cb func(Dimer) (stop bool)) {
	if d == nil {
		return // TODO: coverage
	}
	dd, forDD := []Dimer{d}, (func(func(d Dimer) bool))(nil)
