...
func DimFilling(fw, fh int) *Dim {
	...
	return &Dim{fillsWidth: fw, fillsHeight: fh}
}
...
// height.  Both arguments MUST not be zero.
func DimFillingWidth(fw, h int) *Dim {
	needsToBeNonZero(fw, h)
	return &Dim{fillsWidth: fw, height: h}
}
...
// width.  Both arguments MUST not be zero.
func DimFillingHeight(w, fh int) *Dim {
	needsToBeNonZero(w, fh)
	return &Dim{width: w, fillsHeight: fh}
}
...
// associated Dimer's width is layouted as requested.  NOTE subsequent
// UpdateWidth calls overwrite each other if no Resize was requested
// from the associated layout Manager m in between.   The main use case
// of this feature is that it provides especially for width fillers the
// means to control their filling width.  E.g. an initial layout of a
// Chainer's Dimers might be:
//
//     +------------+------------+
//     |     d      |      d'    |
//     +------------+------------+
...
//     +--------------+----------+
//
// NOTE UpdateWidth calls resulting in a non-positive width are ignored.
...
// associated Dimer's height is layouted as requested.  NOTE subsequent
// UpdateHeight calls overwrite each other if no Reflow was requested
// from the associated layout Manager m in between.   The main use case
// of this feature is that it provides especially for hight fillers the
// means to control their filling height.  E.g. an initial layout of a
// Stacker's Dimers might be the left layout:
//
//     +------------+                           +------------+
//     |            |                           |            |
//     |     d      |                           |     d      |
...
// m.Reflow() will result in the right layout.  NOTE UpdateHeight calls
// resulting in a non-positive height are ignored.
...
// SetHeight sets given positive hight.  Set height filling is removed.
// SetHeight does nothing if given height is not positive.
...
// SetWidth sets given positive width.  Set width filling is removed.
// SetHeight does nothing if given height is not positive.
...
// IsDirty returns true if after a layout reflow the printable area of a
// dimer has changed.
...
// Area provides a Dimer's available area in a layout, i.e. without clippings.
...
// reduced by the clip-width is the available width in a calculated
// layout.
...
// IsOffScreen returns true if clipping height and width are the
// receiving dimension's height and width.
